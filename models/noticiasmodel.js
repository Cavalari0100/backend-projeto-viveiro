...
const conecta = conexao.db("noticia").collection("noticia")
var mongooseConection = require("../infraestrutura/conexaoMongoose")
const {Error} = require("mongoose")
const noticiasmongoose = require("../mongoose/noticiasmongoose")
...
class Noticia{
    ...
    async insertOne(document,res){
        const doc = new noticiasmongoose(document)
        try{
    ...
    async updateOne(id,data){
        const result = await noticiasmongoose.updateOne({ _id: ObjectId(id) }, { $set: data }).then((result, err) => {
            if (err) {
