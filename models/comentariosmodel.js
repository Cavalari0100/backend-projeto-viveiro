...
const conecta = conexao.db("noticia").collection("comentarios")
var mongooseConection = require("../infraestrutura/conexaoMongoose")
const {Error} = require("mongoose")
const comentario = require("../mongoose/comentarios")
...
class Postagem{
    ...
    async insertOne(document,res){
        const doc = new comentario(document)
        try{
    ...
    async updateOne(id,data){
        const result = await comentario.updateOne({ _id: ObjectId(id) }, { $set: data }).then((result, err) => {
            if (err) {
